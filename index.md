...
\>= 1.0, soon to be on CRAN) imports this package and includes a number
of illustrative vignettes. Specifically, the examples demonstrate how to
use existing R functions and optimizers to implement a
homomorphic-encryption protocol across multiple sites.

## Schemes

- **BFV / BGV**: exact integer arithmetic
- **CKKS**: approximate real-number arithmetic (with sin, cos, logistic,
  bootstrapping)
- **FHEW / TFHE (BinFHE)**: Boolean circuit evaluation

All four FHE subsystems (**BFV**, **BGV**, **CKKS**, **BinFHE**) are
exposed. In addition, we provide facilities for serialization, threshold
FHE, CKKS bootstrapping, transcendental function evaluation, and CKKS
