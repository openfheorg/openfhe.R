...
* The package now installs on musl-based Linux (Alpine). The vendored
  ...
  otherwise takes the same branch macOS and Windows always have.
  CRAN's musl check had reported 1.5.1.1 as failing to install.

* `rlang` is declared in `Imports`. The package's error and warning
  messages go through cli functions that call rlang at run time, and
  cli lists rlang only as a suggestion, so on a machine without rlang
  every error read "there is no package called 'rlang'" instead of the
  intended message.
