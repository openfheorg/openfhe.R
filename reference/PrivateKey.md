# Private Key

Private Key

## Usage

``` r
PrivateKey(ptr = NULL)
```

## Arguments

- ptr:

  External pointer (internal use)

## Value

An S7 object of class `PrivateKey`, inheriting from
[OpenFHEObject](https://openfheorg.github.io/openfhe.R/reference/OpenFHEObject.md),
whose `ptr` property holds an external pointer to the C++ secret key.
This is the key
[`decrypt()`](https://openfheorg.github.io/openfhe.R/reference/decrypt.md)
takes; obtain one as the `secret` element of the
[KeyPair](https://openfheorg.github.io/openfhe.R/reference/KeyPair.md)
returned by
[`key_gen()`](https://openfheorg.github.io/openfhe.R/reference/key_gen.md)
rather than by calling this constructor directly.
